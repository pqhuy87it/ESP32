...
def print_datetime(TIME_ZONE):
    """Print the current date and time with the given timezone offset in hours."""
    try:
        # Get the current UTC time as a struct_time tuple
        utc_time = utime.localtime()
        
        # Calculate the adjusted time with the timezone offset (in seconds)
        adjusted_time_in_seconds = utime.mktime(utc_time) + (TIME_ZONE * 3600)
        
        # Convert the adjusted time back to a struct_time tuple
        local_time = utime.localtime(adjusted_time_in_seconds)
        
        # Format the local time into a human-readable format
        formatted_time = "{:04d}-{:02d}-{:02d} {:02d}:{:02d}:{:02d}".format(
            local_time[0], local_time[1], local_time[2],  # Year, Month, Day
            local_time[3], local_time[4], local_time[5]   # Hour, Minute, Second
        )
        
        # Print the current local time with the specified timezone offset
        print("Current Date and Time (UTC+{}): {}".format(TIME_ZONE, formatted_time))
...
def main():
    ...
    while True:
        print_datetime(TIME_ZONE)  # Print time with UTC+6 offset
